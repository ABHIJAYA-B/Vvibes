    ...
    try {
        ...
        if (user.avatar) {
            ...
            fs.unlink(oldAvatarPath, (err) => {
                if (err && err.code !== 'ENOENT') {  // Ignore error if the file is not found
        ...
        avatar.mv(path.join(__dirname, '..', 'uploads', newFilename), async (err) => {
            if (err) {
                return next(new HttpError("Failed to upload the avatar.", 500));
            }
            ...
            // Send response only once after the operation is successful
